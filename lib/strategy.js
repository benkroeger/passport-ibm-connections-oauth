...
Strategy.prototype.authenticate = function authenticate(req, options = {}) {
  ...
  if (req.query && req.query.code) {
    ...
    const getOAuthAccessTokenParams = this.tokenParams(options);
    getOAuthAccessTokenParams.grant_type = 'authorization_code';
    getOAuthAccessTokenParams.callback_uri = callbackURL;

    this._oauth2.getOAuthAccessToken(code, getOAuthAccessTokenParams, (err, accessToken, refreshToken, params) => {
      if (err) {
...
Strategy.prototype.userProfile = function userProfile(accessToken, done) {
  ...
  this._oauth2.get(this._profileURL, accessToken, (err, body, response) => {
    ...
    const setCookies =
      response && response.headers && response.headers['set-cookie'] ? response.headers['set-cookie'] : [];
